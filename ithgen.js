...
function geminate(ca) {
    if(ca.length == 1) {
        ...
        from = ["pt","pk","kt","kp","tk","tp"];
        to   = ["bbḑ","bbv","ggḑ","ggv","ḑvv","ddv"];
        for (i = 0; i < from.length; i++) {
            ca = ca.replace(from[i], to[i]);
        }
        return ca;
    } else if (ca.match(/[ptkbdg][mnň]$/) != null) {
        //pm  vmm pn  vvn kmxxm kn  xxn tm  ḑḑm tn  ḑḑn
        //bm  bžžm bn  bžžn gm gžžm gn  gžžn dm  jjm dn  jjn
        from = ["pm","pn","km","kn","tm","tn","bm","bn","gm","gn","dm","dn"];
        to   = ["vmm","vvn","xxm","xxn","ḑḑm","ḑḑn","bžžm","bžžm","gžžm","gžžn","jjm","jjn"];
        for (i = 0; i < from.length; i++) {
            ca = ca.replace(from[i], to[i]);
        }
        return ca;
    } else if (ca.match(/^[lrř]/)){
        return ca.charAt(0) + ca;
    } else { return ca;}
}
function genWord(opts = {}) {
    ...
    //#region slot 6
        ...
        caFrom = ["pp","tt","kk","ll","pb","kg","çy","řř","rr","rř","řr","çx","gm","gn","ngn","ţţ","bm","bn","ff"];
        caTo   = ["mp","nt","nk","pļ","mb","ng","nd","ňš","ns","nš","ňs","xw","x", "ň", "ňn","ḑy", "f", "ţ", "vw"];
        moreCons = [false,false,false,false,false,false,false,false,false,false,false,true,true,true,false,false,true,true,false];

        for (i = 0; i < caFrom.length; i++) {
            if (!(moreCons[i] && slot6.length == 2)) {
                slot6 = slot6.replace(caFrom[i], caTo[i]);
            }
        }
